...
Syscalls are invoked using the `ECALL` instruction.
...
Arguments to syscalls are passed in `a0` through `a6`, with the syscall index
in `a7`.

Return values from syscalls are returned in `a0` through `a7`.
